...
try {
    ...
    // Retrieve customers who do not have any purchases or have all purchases delivered
    $query = "SELECT * FROM client WHERE id NOT IN (SELECT DISTINCT clientid FROM purchase WHERE delivered = 0)";
    $stmt = $pdo->query($query);
    $customers = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Display the list of customers in a table
    echo "<h1>Customers with No Purchases or All Purchases Delivered</h1>";
